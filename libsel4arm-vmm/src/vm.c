...
#define ICC_PMR_EL1    MSR_MRS_OTHER(3, 0, 4, 6, 0)
#define ICC_IAR0_EL1   MSR_MRS_OTHER(3, 0, 12, 8, 0)
#define ICC_EOIR0_EL1  MSR_MSR_OTHER(3, 0, 12, 8, 1)
#define ICC_HPPIR0_EL1 MSR_MSR_OTHER(3, 0, 12, 8, 2)
#define ICC_BPR0_EL1   MSR_MSR_OTHER(3, 0, 12, 8, 3)
#define ICC_AP0R_EL1   MSR_MSR_OTHER(3, 0, 12, 8, 4)
#define ICC_SGI1R_EL1  MSR_MRS_OTHER(3, 0, 12, 11, 5)


extern char _cpio_archive[];
virq_handle_t vtimer_irq_handle[CONFIG_MAX_NUM_NODES] = {0};
...
static int handle_page_fault(vm_t *vm, fault_t *fault)
{
    ...
    if (d != NULL) {
        if (d->devid == DEV_RAM) {
            DRAMFAULT("[%s] %s fault @ %p from %p\n", d->name,
                      (fault_is_read(fault)) ? "read" : "write", (void *) addr, (void *) pc);
        } else {
            DDEVFAULT("[%s] %s fault @ %p from %p\n", d->name,
                      (fault_is_read(fault)) ? "read" : "write", (void *) addr, (void *) pc);
        }
        return d->handle_page_fault(d, vm, fault);
    } else {
#ifdef CONFIG_ONDEMAND_DEVICE_INSTALL
        addr = fault_get_address(fault) & ~0xfff;
        switch (addr) {
        case 0:
            print_fault(fault);
            return -1;
        default: {
            ...
            DVM("Unhandled fault on address %p\n", (void *)addr);
        }
        }
#endif
        print_fault(fault);
                       ...
                       uintptr_t entry_point_address, uintptr_t context_id)
{
    ...
    printf("Starting VCPU %d on core %lu\n", vm->n_vcpus, target_cpu);
...
static void vtimer_irq_ack(void *token)
{
    ...
    ZF_LOGF_IF(err, "Failed to ACK VTimer: VCPU Ack invocation failed");
}
...
int vm_event(vm_t *vm, seL4_MessageInfo_t tag, seL4_Word badge)
{
    ...
    case seL4_Fault_VMFault: {
        ...
        do {
            ...
            if (err) {
                printf("Error handling page fault\n");
    ...
    case seL4_Fault_VCPUFault: {
      //  printf("%p: Got VCPU fault from badge %p\n", (void *) vcpu_idx, (void *) badge);
        seL4_MessageInfo_t reply;
        ...
        case HSR_EC_WFI_WFE:
            ...
            new_wfi_fault(fault);
            printf("%p: got wfi other\n", (void *) vcpu_idx);

            return 0;
        case HSR_EC_SMC32:
        case HSR_EC_SMC64: {
            int res = handle_smc(vm, vcpu);
            assert(fault_handled(fault));
            return res;
        }
        case HSR_EC_MSR_MRS_OTHER: {
//            printf("%p: got msr mrs other\n", (void *) vcpu_idx);
            new_smc_fault(fault);
...
#if 0
        for (int i = 0; i < vm->n_vcpus; i++) {
            seL4_ARM_VCPU_ReadRegs_t res = seL4_ARM_VCPU_ReadRegs(vm->vcpus[i].vcpu.cptr, seL4_VCPUReg_CNTV_CVAL);
            printf("%d cntv_cval: %lu\n", i, res.value);
            res = seL4_ARM_VCPU_ReadRegs(vm->vcpus[i].vcpu.cptr, seL4_VCPUReg_CNTV_CTL);
            printf("%d cntv_ctl: %lu\n", i, res.value);

        }
#endif
//        printf("%lu: got VTimer event\n", vcpu_idx);
